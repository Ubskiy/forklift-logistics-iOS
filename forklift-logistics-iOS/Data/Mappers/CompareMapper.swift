...
enum CompareMapper {
    static func request(from settings: ScenarioSettings) -> CompareRequestDTO {
        CompareRequestDTO(
            ...
            production: ProductionDTO(c3PerHour: settings.c3RatePerHour),
            annealing: AnnealingDTO(iterations: settings.iterations, seed: settings.seed)
        )
    }
    ...
    static func map(_ dto: CompareResponseDTO) -> CompareResult {
        CompareResult(
            ...
            greedy: strategy(dto.greedy, title: "Жадная стратегия"),
            annealing: strategy(dto.simulatedAnnealing, title: "Имитация отжига"),
            delta: CompareDelta(
